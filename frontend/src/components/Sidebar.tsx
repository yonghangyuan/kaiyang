    ...
    <div style={{ width:380, background:'var(--bg-card)', borderLeft:'1px solid var(--border)', display:'flex', flexDirection:'column' }}>
      <div style={{ display:'flex', borderBottom:'1px solid var(--border)' }}>
        {(['live','feed','chat'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)} style={S.tabBtn(tab===t)}>{t.toUpperCase()}</button>
        ))}
      </div>
      {stats && <div style={{ display:'flex',gap:8,padding:'6px 12px',background:'#131a35',borderBottom:'1px solid var(--border)',fontSize:11 }}>
        <span>{stats.sources}SRC</span>
        <span style={{color:'var(--accent)'}}>{stats.intel}ART</span>
        <span style={{color:'var(--yellow)'}}>{stats.events}EVT</span>
        <span style={{color:'var(--purple)'}}>{stats.entities}ENT</span>
      ...
      <div style={{ flex:1, overflow:'auto', padding:'10px 12px', fontSize:13 }}>
          ...
          profileEntity
            ...
            : <>
                ...
                {briefing?.summary && <div style={{background:'var(--bg-card)',padding:10,borderRadius:6,marginTop:8,borderLeft:'3px solid var(--accent)',fontSize:12,lineHeight:1.6}}>
                  <b style={{fontSize:10,color:'var(--fg-dim)'}}>Search: {briefing.query}</b><br/>
                  {briefing.summary.replace(/\*\*/g,'').replace(/^#{1,4}\s/gm,'').trim()}
                  <div style={{fontSize:10,color:'var(--fg-dim)',marginTop:4}}>{briefing.timeline_count} items</div>
                </div>}
              </>
        )}
        {tab === 'feed' && (
        ...
        {tab === 'chat' && (
          chatMessages.length===0
            ? <div style={{textAlign:'center',padding:40,color:'var(--fg-dim)',fontSize:13}}>Kaiyang AI Assistant</div>

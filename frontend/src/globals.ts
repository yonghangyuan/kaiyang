// Global helpers for Leaflet map popups.
// Must be on `window` because onclick handlers are raw HTML strings.

export function loadEventItems(eventId: string) {
...
export function tianshuPopup(title: string) {
...
// Attach to window for Leaflet inline onclick
(window as any).loadEventItems = loadEventItems
(window as any).tianshuPopup = tianshuPopup
